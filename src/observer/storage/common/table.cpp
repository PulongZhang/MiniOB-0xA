...
RC Table::remove(const char *name)
{
  ...
  for (auto idx: indexes_){
    ...
    if (0!= ::unlink(index_file.c_str())){
      LOG_ERROR("Delete index file failed. filename=%s", index_file.c_str());
      return RC::IOERR;
    }
  }
  indexes_.clear();
  ...
  // drop table resources and data file
  ...
  // Can not delete which may cause double free.
  data_buffer_pool_ = nullptr;

  if (0!=::unlink(data_file.c_str())){
    LOG_ERROR("Delete data file failed. filename=%s", data_file.c_str());
    return RC::IOERR;
  }
  ...
  if (0 != ::unlink(meta_file.c_str())){
    LOG_ERROR("Delete meta file failed. filename=%s", meta_file.c_str());
    return RC::IOERR;
  }
