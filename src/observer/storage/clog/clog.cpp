  ...
  switch (flag) {
    ...
    case REDO_UPDATE: {
      if (!rec || !rec->data()) {
        ...
        log_record_.upd.new_data_len_ = data_len;
        log_record_.upd.hdr_.logrec_len_ = _align8(CLOG_UPD_REC_NODATA_SIZE + data_len);
        log_record_.upd.new_data_ = new char[log_record_.upd.hdr_.logrec_len_ - CLOG_UPD_REC_NODATA_SIZE];
        memcpy(log_record_.upd.new_data_, rec->data(), data_len);
